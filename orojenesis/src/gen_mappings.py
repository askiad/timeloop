...
def run_einsums(chain, output_file, mapping_yaml, arch_yaml, force_rerun=False):
    ...
    for einsum in chain:
        ...
        preprocessed_csv = rundir / "oaves.csv"
        if force_rerun or not preprocessed_csv.exists():
            ...
            utils.RunOAVESPostProcessing(rundir)
        oaves_csv = rundir / "oaves.csv"
...
def gen_mappings(
    ...
    for mapping_postfix in mapping_postfice:
        ...
        for p_order in range(0, P_prob_log + 1):
            ...
            output_subdir = pathlib.Path(
                f"{output_dir}/{arch_prefix}{model_name}/oaves_chain_2levels_outputs{batch}{spatial}{mapping_postfix}_p{p_value}"
            )
            gen_einsums(
                graph_dir,
                config_dir,
